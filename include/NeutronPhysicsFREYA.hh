...
//----------------------------------------------------------------------------
//
#ifndef NeutronPhysicsFREYA_h
#define NeutronPhysicsFREYA_h 1

#include "G4HadronPhysicsFTFP_BERT.hh"
#include "Randomize.hh"

class NeutronPhysicsFREYA : public G4HadronPhysicsFTFP_BERT
{
  public: 
    explicit NeutronPhysicsFREYA(G4int verbose);
    explicit NeutronPhysicsFREYA(const G4String& name);
    explicit NeutronPhysicsFREYA(const G4String& name = "nInelastic FREYA",
                                      G4bool qe = false);
    explicit NeutronPhysicsFREYA(const G4String& name, G4int verbose);
    explicit NeutronPhysicsFREYA(const G4String& name, G4int verbose,
                                      G4double minFTFPEnergy, 
                                      G4double maxBertiniEnergy);

    virtual ~NeutronPhysicsFREYA();
    ...
    NeutronPhysicsFREYA(NeutronPhysicsFREYA &) = delete;
    NeutronPhysicsFREYA & operator =
    (const NeutronPhysicsFREYA &right) = delete;
  ...
  protected:
   ...
   static double rng4llnlfisslib(void){return G4UniformRand();};
};
