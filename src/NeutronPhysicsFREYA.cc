...
G4_DECLARE_PHYSCONSTR_FACTORY(NeutronPhysicsFREYA);


NeutronPhysicsFREYA::NeutronPhysicsFREYA(G4int verb)
  :  NeutronPhysicsFREYA()
{
  G4HadronicParameters::Instance()->SetVerboseLevel(verb);
} 

NeutronPhysicsFREYA::NeutronPhysicsFREYA(const G4String& name)
  :  NeutronPhysicsFREYA(name, false)
{} 

NeutronPhysicsFREYA::NeutronPhysicsFREYA(const G4String& name, G4bool qe)
  : G4HadronPhysicsFTFP_BERT(name, qe), useLEND_(false), evaluation_("")
{
  minBERT_neutron = 19.9*CLHEP::MeV;
}

NeutronPhysicsFREYA::NeutronPhysicsFREYA(const G4String& name, G4int verb)
  :  NeutronPhysicsFREYA(name, false)
{
  G4HadronicParameters::Instance()->SetVerboseLevel(verb);
} 
...
NeutronPhysicsFREYA::~NeutronPhysicsFREYA()
{}

void NeutronPhysicsFREYA::Neutron()
{
...
/*
  ...
  G4HadronicProcess* fission = G4PhysListUtil::FindFissionProcess(G4Neutron::Neutron());
  if (fission) {
	  G4NeutronHPFissionData* dataSet4 = new G4NeutronHPFissionData();
          fission->AddDataSet(dataSet4);
...
#ifdef USEFREYA
   G4cout << "NeutronHPphysics: using new version of LLNL Fission Library (overriding built-in version) with FREYA turned on"<<G4endl;
...
#endif
   fission->RegisterMe(theFissionModel);
  }
}

void NeutronPhysicsFREYA::ConstructProcess()
{
